import { _decorator, Color, Component, game, Label, Node, Sprite, SpriteFrame } from 'cc';
import { VirtualScrollView } from '../../VScrollView';
import UIButton from './UIButton';
const { ccclass, property } = _decorator;

@ccclass('scene8')
export class scene8 extends Component {
  ...
  onLoad() {
    ...
    if (this.vlist) {
      this.vlist.renderItemFn = (itemNode: Node, index: number) => {
        ...
        if(this.data[index].clicked){
          title!.color = new Color(255,0,0);
        }else{
          title!.color = new Color(75,75,75);
        }
      ...
      this.vlist.onItemClickFn = (itemNode: Node, index: number) => {
        const tip = this.node.getChildByName('tip').getComponent(Label);
        tip.string = `邮件内容:${this.data[index].data1}`;
