...
export class VirtualScrollView extends Component {
	...
	public scrollToIndex(index: number, animate = false, duration?: number, onComplete?: () => void, center = false) {
		...
		if (this.useDynamicSize) {
			...
			targetPos = center
				? itemStart - (this._viewportSize / 2) + (itemSize / 2)
				: itemStart;
		} else {
			...
			targetPos = center
				? itemStart - (this._viewportSize / 2) + (this.itemMainSize / 2)
				: itemStart;
		}
	...
	private _onUp(e?: EventTouch) {
		...
		if (this._velSamples.length >= 2) {
			...
			if (dtSum > 0.001) {
				...
				this._velocity =
					this._velSamples.length > 0
						? math.clamp(this._velSamples[this._velSamples.length - 1].delta * 60, -this.maxVelocity, this.maxVelocity)
						: 0;
			}
